...
class Validator implements \M2E\TikTokShop\Model\TikTokShop\Listing\Product\Action\Type\ValidatorInterface
{
    ...
    public function validate(
        ...
        if (!$actionConfigurator->isVariantsAllowed()) {
            $this->addErrorMessage(
                new Action\Validator\ValidatorMessage(
                    (string)__('The product was not relisted because it has no associated products.'),
                    \M2E\TikTokShop\Model\Tag\ValidatorIssues::ERROR_NO_ASSOCIATED_PRODUCTS_RELIST
                )
            );
