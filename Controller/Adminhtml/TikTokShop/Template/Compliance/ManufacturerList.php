<?php

declare(strict_types=1);

namespace M2E\TikTokShop\Controller\Adminhtml\TikTokShop\Template\Compliance;

class ManufacturerList extends \M2E\TikTokShop\Controller\Adminhtml\TikTokShop\AbstractTemplate
{
    private \M2E\TikTokShop\Model\Template\Compliance\ComplianceService $complianceService;
    private \M2E\TikTokShop\Model\Account\Repository $accountRepository;

    public function __construct(
        \M2E\TikTokShop\Model\Template\Compliance\ComplianceService $complianceService,
        \M2E\TikTokShop\Model\Account\Repository $accountRepository,
        \M2E\TikTokShop\Model\TikTokShop\Template\Manager $templateManager
    ) {
        parent::__construct($templateManager);

        $this->accountRepository = $accountRepository;
        $this->complianceService = $complianceService;
    }

    public function execute()
    {
        $accountId = (int)$this->getRequest()->getParam('account_id');
        $account = $this->accountRepository->find($accountId);

        if ($account === null) {
            $this->setJsonContent([
                'result' => false,
                'message' => 'Account Id is required',
            ]);

            return $this->getResult();
        }

        $force = (bool)(int)$this->getRequest()->getParam('force', 0);

        $manufacturers = [];
        foreach ($this->complianceService->getAllManufacturers($account, $force)->getAll() as $manufacturer) {
            $manufacturers[] = [
                'id' => $manufacturer->id,
                'title' => $manufacturer->name,
            ];
        }

        $this->setJsonContent([
            'result' => true,
            'manufacturers' => $manufacturers,
        ]);

        return $this->getResult();
    }
}
