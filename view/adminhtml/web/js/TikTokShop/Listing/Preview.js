define(function () {
    ...
    window.TikTokShopListingPreviewItems = Class.create(Common, {

        initialize: function () {
            var self = this;

            jQuery('.price_container button').click(function () {
                self.alert(TikTokShop.translator.translate('This is Item Preview Mode'));
            });
        ...
        getVariation: function (chosenOptions) {
            ...
            TikTokShop.formData.variations.variations.forEach(function (variation) {
        ...
        onChangeVariationSelect: function (currentSelect) {
            ...
            if (choseSelects === $$('select').length - 1) {
                ...
                $$("select[value=]")[0].select('option[value!=]').each(function (option) {
                    ...
                    variation = TikTokShopListingPreviewItemsObj.getVariation(otherSelectsValues);
            ...
            if (TikTokShop.formData.images.variations &&
                    TikTokShop.formData.images.variations.specific === currentSelect.name && currentSelect.value) {
                $('product_image').src = TikTokShop.formData.images.variations.images[currentSelect.value][0];
            }
        },
        ...
        initVariations: function () {
            $$('select').each(function (currentSelect) {
                if (currentSelect.select('option[value!=]').length === 1) {
                    ...
                    TikTokShopListingPreviewItemsObj.onChangeVariationSelect(currentSelect);
                }
            });
        }
    });
